...
const _entryColumns =
    'id, content, language, link_type, link_id, link_label, entry_kind, title, created_at, updated_at';
...
class NotebookRepository {
  ...
  Future<List<NotebookEntry>> fetchAll() async {
    ...
    return (rows as List).map((row) => NotebookEntry.fromRow(row as Map<String, dynamic>)).toList();
  }
  ...
  Future<void> delete(String entryId) async {
    await supabase.from('student_notebook_entries').delete().eq('id', entryId).eq('student_id', _studentId);
  }
}
