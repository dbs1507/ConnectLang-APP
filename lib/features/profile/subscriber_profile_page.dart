...
import '../placement/placement_home_page.dart';
import '../vocabulary/models/vocabulary_entry.dart';
import 'profile_repository.dart';

/// Espelha a aba "study" de `SubscriberProfilePage.tsx`, reduzida: sem o
/// aviso de cooldown de retake (`fetchPlacementRetakeCooldown`, fora do
/// escopo do Nivelamento no app) e sem `SubscriberSkillProgress`. Refazer o
/// nivelamento leva pra tela de escolha de idioma em vez de pular direto
/// pro idioma da linha (o app ainda não aceita idioma pré-selecionado ali).
class SubscriberProfilePage extends ConsumerStatefulWidget {
  const SubscriberProfilePage({super.key});

  @override
  ConsumerState<SubscriberProfilePage> createState() => _SubscriberProfilePageState();
}
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            Text('Idiomas de estudo', style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 12),
            Wrap(
              spacing: 8,
              runSpacing: 8,
            ...
            if (_savedOk) ...[
              const SizedBox(height: 12),
              Text('Salvo!', style: TextStyle(color: Theme.of(context).colorScheme.primary)),
            ],
            if (_error != null) ...[
              const SizedBox(height: 12),
            ...
            Text('Nível por idioma', style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 12),
            if (user.studyLanguages.isEmpty)
              const Text('Defina um idioma de estudo pra ver o nível.')
            else
              for (final code in user.studyLanguages) _LevelRow(code: code, user: user),
          ],
        ),
      ),
    );
...
class _LevelRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final level = user.cefrLevelByLanguage[code];
    return Card(
      margin: const EdgeInsets.only(bottom: 8),
      child: ListTile(
        title: Text(taughtLanguages[code] ?? code),
        subtitle: Text(level != null ? 'Nível $level' : 'Nivelamento pendente'),
        trailing: TextButton(
