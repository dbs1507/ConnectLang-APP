...
/// `src/lib/vocabEnrich.ts` — modo "full" pra palavra ou frase.
...
String buildEnrichedTranslationField(
  String translation,
  List<String>? alternates,
) {
  final fromMain = translation
      .split(RegExp(r'\s*·\s*'))
      .map((s) => s.trim())
      .where((s) => s.isNotEmpty);
  final fromAlts = (alternates ?? [])
      .map((s) => s.trim())
      .where((s) => s.isNotEmpty);
  final seen = <String>{};
  final parts = <String>[];
...
class VocabEnrichResult {
  ...
  factory VocabEnrichResult.fromRow(Map<String, dynamic> row) {
    ...
    final rawAlts =
        row['translationAlternates'] ?? row['translation_alternates'];
    final alternates = rawAlts is List
